...
export default function UserProfile({ user, conversations, stats, isEditing, setIsEditing, onUpdateProfile }) {
    ...
    useEffect(() => {
        if (user) {
            setFormData(prev => ({
                ...
                avatar_preview: null, profile_bg_image_preview: null
            }));
        }
    }, [user]);
    ...
    const handleRemovePhoto = (type) => {
        if (type === 'avatar') {
            setFormData(prev => ({ ...prev, avatar_file: null, avatar_preview: '/images/default-avatar.png', remove_avatar: true }));
        } else if (type === 'bg') {
            setFormData(prev => ({ ...prev, profile_bg_image_file: null, profile_bg_image_preview: 'none', profile_bg_image: null, remove_profile_bg: true }));
        }
    };
        ...
        <div className="flex flex-col text-[#222] bg-[#F3F3F3] min-h-screen relative font-sans">
            ...
            <div className="min-h-[450px] relative flex flex-col items-center justify-center pt-20 pb-12 overflow-hidden bg-cover bg-center"
                ...
                {isEditing && (
                    <div className="absolute top-0 left-0 w-full bg-black/60 p-4 flex justify-between items-center z-50 text-white">
                        ...
                        <div className="flex gap-2">
                            <label className="bg-white/10 px-4 py-2 rounded-full text-xs font-bold border border-white/20 cursor-pointer">
                                Ganti BG <input type="file" name="profile_bg_image" className="hidden" onChange={handleFileChange} accept="image/*" />
                            </label>
                            <button onClick={handleSubmit} disabled={loading} className="bg-[#ff6122] px-6 py-2 rounded-full text-sm font-bold">Simpan</button>
                ...
                <div className="z-10 flex flex-col items-center w-full max-w-4xl px-6">
                    <div className="w-40 h-40 rounded-full border-[6px] border-white/20 overflow-hidden shadow-2xl relative mb-8">
                        <img src={formData.avatar_preview || user.avatar || '/images/default-avatar.png'} className="w-full h-full object-cover" alt="" />
                        {isEditing && (
                            <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center gap-2">
                                ...
                                <button onClick={() => handleRemovePhoto('avatar')} className="text-white text-[10px] font-bold bg-red-600 px-3 py-1.5 rounded-full">Hapus</button>
                            </div>
                        )}
                    </div>
